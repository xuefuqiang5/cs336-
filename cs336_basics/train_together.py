...
def maybe_save_checkpoint(args, model, optimizer, iteration):
...
def train(args, train_data_loader, valid_data_loader, model, optimizer):
    ...
    for batch_idx, (x, y) in enumerate(tqdm(train_data_loader, mininterval=30.0)):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            val_loss = evaluate_model(args, valid_data_loader, model)["val_loss"]
            model.train()
            print(
                f"Batch {batch_idx} | "
                f"Loss: {avg_loss:.4f} | "
        ...
        if save_interval and (batch_idx + 1) % save_interval == 0:
            maybe_save_checkpoint(args, model, optimizer, batch_idx + 1)
    maybe_save_checkpoint(args, model, optimizer, batch_idx + 1)
